...
// token IDs — one per keyword/operator so bison can use them directly later
// sentinels (_END) mark group boundaries for tokenCategory()
enum TokenID {
    // literals & identifiers
    TOK_IDENTIFIER = 258,   // above ASCII range (bison convention)
    TOK_INT_LITERAL,
    TOK_FLOAT_LITERAL,
    TOK_CHAR_LITERAL,
    TOK_STRING_LITERAL,
...
// maps token id -> category name for the output table
...
// reserved word table — add new keywords here + in the enum above
// not keywords: static, typedef, enum, union (not in our language)
// not keywords: printf, scanf (they're just identifiers)
