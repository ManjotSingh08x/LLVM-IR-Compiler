...
enum TokenID {
    ...
    TOK_NEW, TOK_DELETE, TOK_TRUE, TOK_FALSE, TOK_THIS, TOK_SIZEOF,
    ...
    TOK_LBRACKET, TOK_RBRACKET, TOK_SEMI, TOK_COMMA, TOK_DOT, TOK_COLON,
...
inline std::string tokenCategory(int tok) {
    ...
    if (tok >= TOK_INT && tok <= TOK_SIZEOF) return "keyword";
    if (tok >= TOK_PLUS && tok <= TOK_SCOPE) return "operator";
    if (tok >= TOK_LPAREN && tok <= TOK_COLON) return "punctuation";
    if (tok == TOK_LEX_ERROR) return "lexical_error";
    return "unknown";
}
...
inline const std::unordered_map<std::string, int>& keywordTable() {
    static const std::unordered_map<std::string, int> kw = {
        ...
        {"false", TOK_FALSE}, {"this", TOK_THIS}, {"sizeof", TOK_SIZEOF}
    };
    return kw;
}
